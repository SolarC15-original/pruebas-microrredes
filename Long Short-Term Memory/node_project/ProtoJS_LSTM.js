/**
 * ProtoJS_LSTM.js — Prototipo LSTM en Node.js/tfjs-node
 * Carga dataset real de Kaggle + scaler params exportados por Python,
 * entrena modelo LSTM, mide latencia de inferencia y memoria.
 */
require('./patchnode');
...
function buildLSTM(inputShape) {
  const inputs = tf.input({ shape: inputShape });
  let x = tf.layers.lstm({
    units: 64, returnSequences: true, activation: 'tanh'
  }).apply(inputs);
  x = tf.layers.dropout({ rate: 0.3 }).apply(x);
  x = tf.layers.lstm({
    units: 32, returnSequences: false, activation: 'tanh'
  }).apply(x);
  x = tf.layers.dropout({ rate: 0.3 }).apply(x);
  x = tf.layers.dense({ units: 16, activation: 'relu' }).apply(x);
  const outputs = tf.layers.dense({ units: 3 }).apply(x);
  return tf.model({ inputs, outputs, name: 'LSTM_Microgrid_JS' });
}
...
async function main() {
  ...
  console.log('PROTOTIPO LSTM (NODE.JS/TFJS) — MICRORREDES');
  console.log('='.repeat(65));
  ...
  if (!model) {
    model = buildLSTM([SEQ_LEN, featureCols.length]);
    model.summary();
  ...
  if (fs.existsSync(pyMetricsPath)) {
    try {
      ...
      if (pyMetrics.BiLSTM_Microgrid) {
        pyTrainEpoch = pyMetrics.BiLSTM_Microgrid.time_per_epoch_s.toFixed(3);
      }
    } catch (e) {}
  }
  ...
  console.log('  ' + '-'.repeat(55));
  console.log('  *Ejecutar ProtoPy_LSTM.py primero para generar scaler_params.json + metrics_python.json');
