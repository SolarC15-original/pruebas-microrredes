"""
graficas_LSTM.py — Visualización comparativa Python vs JavaScript (tfjs-node)
Métricas de inferencia, throughput y memoria para microrredes con LSTM.
...
if py_metrics is None or js_metrics is None:
    print("ERROR: Ejecutá ProtoPy_LSTM.py y ProtoJS_LSTM.js primero.")
    sys.exit(1)

py_inf = py_metrics.get('inference', {})
py_model = py_metrics.get('BiLSTM_Microgrid', py_metrics.get('LSTM_Microgrid', {}))
...
fig.suptitle('Benchmark LSTM — Python vs Node.js (tfjs-node)\nMicrorredes Eléctricas — CPU (sin GPU)',
