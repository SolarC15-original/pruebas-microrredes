/**
 * ProtoJS_TFT.js — Prototipo TFT (Temporal Fusion Transformer) en Node.js/tfjs-node
 */
require('./patchnode');
...
function createSequences(rows, featureCols, targetCols, seqLen) {
  ...
  return { data: tf.tensor3d(X), targets: tf.tensor2d(y) };
}
...
function buildTFT(inputShape) {
  const inputs = tf.input({ shape: inputShape });
  const dModel = 64;

  // Feature projection + feature gating
  let x = tf.layers.dense({ units: dModel }).apply(inputs);
  const gate = tf.layers.dense({ units: dModel, activation: 'sigmoid' }).apply(inputs);
  x = tf.layers.multiply().apply([x, gate]);

  // LSTM encoder
  const lstm = tf.layers.lstm({ units: dModel, returnSequences: true }).apply(x);
  x = tf.layers.add().apply([x, lstm]);
  x = tf.layers.layerNormalization().apply(x);

  // Second LSTM layer (compensa la falta de MultiHeadAttention en tfjs-node)
  const lstm2 = tf.layers.lstm({ units: dModel, returnSequences: true }).apply(x);
  x = tf.layers.add().apply([x, lstm2]);
  x = tf.layers.layerNormalization().apply(x);
  ...
  return tf.model({ inputs, outputs, name: 'TFT_Microgrid_JS' });
}

async function trainJS(model, xs, ys, epochs = 20) {
  model.compile({ optimizer: 'adam', loss: 'meanSquaredError', metrics: ['mae'] });
  const t0 = perf_hooks.performance.now();
  const history = await model.fit(xs, ys, {
    epochs, batchSize: 64, validationSplit: 0.1, verbose: 0,
  });
...
async function benchmarkInference(model, xs, nRuns = 500) {
  ...
  for (let i = 0; i < 10; i++) {
    const input = xs.slice([i % nSamples, 0, 0], [1, xs.shape[1], xs.shape[2]]);
    model.predict(input);
    input.dispose();
    tf.dispose();
  }
  ...
  return { mean, std, p50, p95, p99, min: latencies[0], max: latencies[latencies.length - 1], nRuns };
}
...
async function loadExportedModel(modelDir = '../exported_model') {
  ...
  if (fs.existsSync(modelJson)) {
    ...
    return await tf.loadLayersModel(`file://${modelJson}`);
  }
...
async function main() {
  ...
  console.log('PROTOTIPO TFT (NODE.JS/TFJS) — MICRORREDES');
  console.log('='.repeat(65));
  ...
  if (!model) {
    model = buildTFT([SEQ_LEN, featureCols.length]);
    model.summary();
  ...
  const metricsJs = {
    model: {
      ...
      test_loss_mse: testLoss, test_mae: testMae,
    },
    inference: {
      mean_ms: infStats.mean, std_ms: infStats.std, p50_ms: infStats.p50,
      p95_ms: infStats.p95, p99_ms: infStats.p99,
  ...
  if (fs.existsSync(pyMetricsPath)) {
    try {
      ...
      if (pyMetrics.TFT_Microgrid) {
        pyTrainEpoch = pyMetrics.TFT_Microgrid.time_per_epoch_s.toFixed(3);
      }
    } catch (e) {}
  }

  console.log('\n' + '='.repeat(65));
  console.log('COMPARATIVA PYTHON vs JAVASCRIPT — TFT');
  console.log('='.repeat(65));
