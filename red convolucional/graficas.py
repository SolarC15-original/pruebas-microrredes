"""
graficas.py — Visualización comparativa Python vs JavaScript (tfjs-node)
Métricas de inferencia, throughput y memoria para microrredes.
...
py_path = os.path.join(os.path.dirname(__file__), 'exported_model', 'metrics_python.json')
js_path = os.path.join(os.path.dirname(__file__), 'exported_model', 'metrics_js.json')

py_metrics = load_metrics(py_path, 'Python')
js_metrics = load_metrics(js_path, 'Node.js')

if py_metrics is None or js_metrics is None:
    print("ERROR: Ejecutá ProtoPy.py y ProtoJS.js primero para generar las métricas.")
    sys.exit(1)

# Extraer valores (usar TCN de Python para comparación)
py_inf = py_metrics.get('inference', {})
py_model = py_metrics.get('TCN_Microgrid', {})
...
python_vals = [metrics[c]['Python'] for c in categories]
node_vals   = [metrics[c]['Node.js'] for c in categories]
...
# --- Panel A: Latencia de inferencia ---
ax = axes[0]
inf_cats = ['Inferencia\nMedia (ms)', 'Inferencia\nP95 (ms)']
...
fig.suptitle('Benchmark 1D-CNN — Python vs Node.js (tfjs-node)\nMicrorredes Eléctricas — CPU (sin GPU)',
...
# ─────────────────────────────────────────────
# FIGURA 2: Ratio de mejora (Node.js vs Python)
# ─────────────────────────────────────────────
fig2, ax2 = plt.subplots(figsize=(10, 5))

ratio_cats = ['Inferencia\nMedia', 'Inferencia\nP95', 'Training\ns/epoch', 'Throughput', 'Memoria\nRSS']
# Para latencia/training: menor es mejor -> Python/Node
# Para throughput: mayor es mejor -> Node/Python
# Memoria: menor es mejor
...
ratios = [
    inf_mean_py / inf_mean_js if inf_mean_js > 0 else 0,  # Inferencia media
    inf_p95_py / inf_p95_js if inf_p95_js > 0 else 0,     # Inferencia P95
    train_py / train_js if train_js > 0 else 0,            # Training
    tp_js / tp_py if tp_py > 0 else 0,                     # Throughput
    mem_js / 190,                                          # Memoria (190 MB Python estimado)
]
